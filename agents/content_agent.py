...
class ContentAgent:
    def draft_article(self, candidate: Candidate, context: Dict[str, Any], verified_facts: Dict[str, Any]) -> ArticleDraft:
        ...
        system_prompt = (
            ...
            "MANDATORY SECTION SKELETON (follow this exact order, using <h2> for top-level and <h3> for sub-sections. ALL <h2> and <h3> tags MUST include a descriptive 'id' attribute for anchor linking):\n"
            "1. <h2>Table of Contents</h2> - A <ul> list of clickable anchor links (e.g. <a href=\"#features\">) to all the <h2> sections below.\n"
            "2. <h2>What is {game_name}?</h2> - 2-3 short paragraphs introducing the game/platform.\n"
            "3. <h2>Features of {game_name}</h2> with these <h3> sub-sections, each 3-5 sentences:\n"
            ...
            "   - Promotional Offers\n"
            "   - Customer Support\n"
            "4. <h2>Pros and Cons</h2> with <h3>Pros</h3> and <h3>Cons</h3>, each a <ul> of 4-6 bullet points.\n"
            "5. <h2>How to Get Started on {game_name}</h2> with <h3>Register</h3>, <h3>Login Process</h3>, "
            "and <h3>Download</h3>, each as a numbered <ol> step list.\n"
            "6. <h2>How to Deposit & Withdraw Money</h2> with <h3>Deposit Money</h3> and <h3>Withdraw Winnings</h3>, "
            "each as a numbered <ol> step list.\n"
            "7. <h2>Games/Bet Types Available</h2> as a <ul> list, each item bolded name + one-sentence description.\n"
            "8. <h2>Rewards and Bonuses</h2> - only include bonus types present in Verified Facts, each as an <h3> "
            "with a 1-2 sentence description. Do not list a bonus type unless it appears in Verified Facts.\n"
            "9. <h2>Personal Review</h2> - MUST begin the first sentence with the exact phrase 'By our expert,' "
            "and read as a first-person hands-on assessment (5-7 sentences).\n"
            "10. <h2>Who This Game Suits</h2> - MUST use this exact heading text. 3-4 sentences on the ideal player type.\n"
            "11. <h2>How It Compares</h2> - MUST include the word 'comparison' and briefly contrast against one other "
            ...
            "12. <h2>FAQs</h2> - exactly 3 question/answer pairs as <h3>Question</h3><p>Answer</p>.\n"
            "13. <h2>Conclusion</h2> - 2 short paragraphs, include a responsible-gaming reminder in the second paragraph.\n\n"
            "RULES:\n"
            ...
            "- Do not omit any of the 13 sections above, even if brief.\n"
            "- Section 9 must literally contain the substring 'By our expert,' (case-insensitive) for it to pass "
            "downstream linting.\n"
            "- Section 10 must literally contain the substring 'Who This Game Suits' (case-insensitive).\n"
            "- Section 11 must literally contain the substring 'comparison' (case-insensitive).\n"
        )
